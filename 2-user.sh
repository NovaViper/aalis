...
if [[ "$use_graphics" == "yes" ]]; then
	...
	if [[ "yes" == $(askYesNo "Do you want to a GUI package manager?") ]]; then
		while true; do
			read -p "$(output ${YELLOW} "What Package GUI do you want to install? Manjaro's [P]amac tool, [O]ctopi: ")" manager
			case $manager in
			P | p)
				...
				break;;
			O | o)
				output ${YELLOW} "Installing Octopi"
				installYay "octopi"
				break;;
	...
	if [[ "$is_laptop" == "yes" ]]; then
		banner ${LIGHT_PURPLE} "Installing Laptop specific tools"
